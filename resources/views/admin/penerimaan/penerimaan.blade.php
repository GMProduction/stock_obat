        ...
        <div class="grid grid-cols-1 gap-4">
            <div class="section relative">
                ...
                <div class="absolute right-0 top-0 mt-3 mr-3">
                    <div class="flex">
                        <button
                            class="bg-green-500 rounded-md flex items-center text-white px-3 py-2 text-sm mr-3"><span
                ...
                <table id="tb-master" class="stripe hover mt-10"
                    ...
                    {{--                            <button--}}
                    {{--                                class="bg-secondary rounded-full text-white px-3 py-2 btn-tambahMaster text-xs">Ubah--}}
                    {{--                            </button>--}}
                    {{--                            <button class="bg-red-500 rounded-full text-white px-3 py-2 text-xs"--}}
                    {{--                                    onclick="confirmDelete(function(){alert('ok')}, function(){alert('cancel')})">Hapus--}}
                    {{--                            </button>--}}
                    {{--                        </td>--}}
                    {{--                    </tr>--}}
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = BasicDatatableGenerator('#tb-master', path, [
                ...
                {
                    data: 'date', name: 'date', className: 'text-center text-xs', render: function (data) {
                        let date = new Date(data);
                        return date.toLocaleString('id-ID', {day: 'numeric', month: 'long', year: 'numeric'});
                    }
                },
                {data: 'batch_id', name: 'batch_id', className: 'text-center text-xs'},
                {data: 'budget_source.name', name: 'budget_source.name', className: 'text-left text-xs'},
                {
                    className: 'text-center text-xs font-bold flex flex-nowrap gap-1 justify-center',
                    searchable: false, orderable: false,
                    data: null, render: function (data) {
                        return '<button data-id="' + data['id'] + '" class="bg-secondary rounded-full text-white px-3 py-2 btn-detail text-xs">Detail</button>';
                    }
                },
            ]);
